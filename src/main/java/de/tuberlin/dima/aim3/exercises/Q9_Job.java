...
//                .filter(item -> item.f3<1990)
        ...
        var join = title_principals
                ...
                .projectSecond(1).projectFirst(5,0)
        ...
        var intermediate_map = //join.map(item -> new Tuple2<String, String>(item.getField(0), item.getField(1))).returns(Types.TUPLE(Types.STRING, Types.STRING));
        join.map(item -> new Tuple4<String, String, String,Integer>(item.getField(0), item.getField(1),item.getField(2),1)).returns(Types.TUPLE(Types.STRING, Types.STRING, Types.STRING, Types.INT))
        ...
        var intermediate_map2 =
                join2.map(item -> new Tuple4<String, ArrayList<String>, Integer, ArrayList<String>>(item.getField(0),item.getField(1),item.getField(2), new ArrayList<String>(Arrays.asList(new String[]{item.getField(3)})))).returns(Types.TUPLE(Types.STRING, Types.LIST(Types.STRING), Types.INT, Types.LIST(Types.STRING)));

        var query= intermediate_map2
                .groupBy(0)
                .reduce((item1, item2) ->{
