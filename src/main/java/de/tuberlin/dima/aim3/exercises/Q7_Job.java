...
public class Q7_Job {
    public static void main(String[] args) throws Exception {
        ...
        var title_principals_filtered =
                title_principals.filter(item -> item.f3.equals("actor"));

        var join = title_principals_filtered
        ...
        var title_akas_mapped = title_akas.map(item -> new Tuple2<String,String>(item.f0,item.f2)).returns(Types.TUPLE(Types.STRING, Types.STRING));

        var join3 = join2.join(title_akas_mapped)
                .where(item -> item.f0)
                .equalTo(item -> item.f0)
                .projectSecond(1)
